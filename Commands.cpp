...
void CopyCommand::execute() {
    ...
    std::ifstream in(c_file, std::ifstream::ate | std::ifstream::binary);
    int l = in.tellg();
...
void ExternalCommand::execute() {
    this->jobsList->removeFinishedJobs();
    std::string trimed = _trim(string(this->cmd_line));
    char* CMDLINE = new char[80];
    strcpy(CMDLINE, trimed.c_str());
    bool is_bg = _isBackgroundComamnd(this->cmd_line);
    if(is_bg){
